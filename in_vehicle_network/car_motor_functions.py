...
def init_gpio():
    ...
    if app_conf.debug_gpio:
        print(
            "gpio control functions:  init_gpio  standby pin ", obu.standby, "OUT LOW"
        )
    ...
    if app_conf.debug_gpio:
        print(
            "gpio control functions:  init_gpio  pwm_tm ",
            obu.pwm_tm,
            "pwm_dm ",
            obu.pwm_dm,
            "OUT LOW",
...
def init_pwm(speed, pwm_tm, pwm_dm):
    ...
    if app_conf.debug_gpio:
        print(
            "gpio control functions:    init_pwn  pwm_tm_control ",
            pwm_tm_control,
            "   speed ",
            speed,
        )
    ...
    if app_conf.debug_gpio:
        print(
            "gpio control functions:    init_pwn  ipwm_dm_control",
            pwm_dm_control,
            "   speed ",
            speed,
        )
...
def deactivate():
    ...
    if app_conf.debug_gpio:
        print(
            "gpio control functions:   deactivate   standby ",
            obu.__file__standby,
            " LOW",
        )
...
def move(on, off, pwm):
    ...
    if app_conf.debug_gpio:
        print(
            "gpio control functions:    move  pwm ",
            pwm,
            " HIGH     pin1 ",
            off,
            " LOW   pin 2",
...
def stop(in1, in2, pwm):
    ...
    if app_conf.debug_gpio:
        print(
            "gpio control functions:    stop  pwm ",
            pwm,
            " LOW     pin1 ",
            in1,
            " LOW   pin 2",
...
def change_speed(speed, pwm_control):
    ...
    if app_conf.debug_gpio:
        print(
            "gpio control functions:   change_speed pwm ", pwm_control, " speed ", speed
        )
    return speed
...
def open_vehicle(obd_2_interface):
    ...
    init_gpio()
    pwm_tm_control, pwm_dm_control = init_pwm(
        obd_2_interface["speed"], obu.pwm_tm, obu.pwm_dm
    )
...
def new_speed(move_command, obd_2_interface, pwm_control):
    ...
    if move_command == "i":
        obd_2_interface["speed_var"] = obd2.speed_inc
    elif move_command == "d":
        obd_2_interface["speed_var"] = obd2.speed_dec
    elif move_command == "i2":
        obd_2_interface["speed_var"] = obd2.speed_inc + fast_speed_update
    elif move_command == "d2":
        obd_2_interface["speed_var"] = obd2.speed_dec - fast_speed_update
...
def set_vehicle_complete_info(
    speed, speed_var, dir, steer_wheel, head, vehicle_st, obd_2_interface
):
