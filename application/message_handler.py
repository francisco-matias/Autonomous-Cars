...
def trigger_event(node_type, event_number, event_status):
	...
	if (node_type == map.obu_node):
		...
		# print(f"[DEBUG] Triggering event: type={event_type}, number={event_number}, status={event_status}\n")
		...
		if (event_type == app_rsu_conf.event_type[event_number]):
			# print(f"Event type: {event_type}, Event ID: {event_number}\n")
...
def spat_generation(rsu_interface):
    """Gera mensagens SPAT com suporte para priorização."""
    if rsu_interface["type"] == map.rsu_node:
        priority_request = rsu_interface.get("priority_request", False)

        # Alteração para indicar prioridade no semáforo
        if priority_request:
            for direction, signal in rsu_interface.get("tls_group", {}).items():
                if signal['state'] != 'green':  # Alterar para verde apenas se não estiver
                    signal['state'] = 'green'
                    # print(f"Traffic light for direction {direction} set to green due to priority request.")
        ...
        spat = {
            ...
            "priorityInformation": {
                "priorityRequest": priority_request
            }
        }
        return spat
    return None
...
def update_traffic_light_priority(rsu_interface, msg):
    """Atualiza o estado do semáforo baseado no DEN message da ambulância."""
    if msg['event']['event_type'] == 'emergency_vehicle_approaching':
        rsu_interface['priority_request'] = True
        print(f"** AMBULANCIA TEM PRIORIDADE NO SEMAFORO **\n") 
