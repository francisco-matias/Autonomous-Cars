...
def main(argv):
	...
	try:
		...
		if (node_type==maps.obu_node):
			...
			rsu_interface = {}  # Inicializar como um dicionário vazio
			t = Thread(target=obu_application_txd, args=(node_interface, start_flag, my_system_rxd_queue, ca_service_txd_queue, den_service_txd_queue, None,))
			t.start()
			threads.append(t)
			...
			#             	my_system_rxd_queue: queue to send data to my_system that is relevant for business logic decision-process 
			t = Thread(target=obu_application_rxd, args=(node_interface, start_flag, services_rxd_queue, my_system_rxd_queue, rsu_interface,))
			t.start()
			threads.append(t)
			...
			t=Thread(target=obu_system, args=(node_interface, start_flag, coordinates, my_system_rxd_queue, movement_control_txd_queue, den_service_txd_queue, rsu_interface))
			t.start()
			threads.append(t)
		...
		if (node_type==maps.rsu_node):
			...
			rsu_interface = {
				'node_id': node_id,
				'type': node_type,
				'num_tls': maps.map[node_id]['num_tls'],
				'tls_group': maps.map[node_id]['tls_groups'],
				'movement': maps.map[node_id]['movement'],
				'plus_info': plus_info,
				'time': current_time,
				'priority_request': False,  # Inicializa sem prioridade
    			}
			...
			#             	services_rxd_queue: queue to get data from ca_service_rxd or den_service_rxd
			#             	my_system_rxd_queue: queue to send data to my_system that is relevant for business logic decision-process 
			t = Thread(target=rsu_application_rxd, args=(node_interface, start_flag, services_rxd_queue, my_system_rxd_queue,rsu_control_txd_queue,))
			t.start()
			threads.append(t)
